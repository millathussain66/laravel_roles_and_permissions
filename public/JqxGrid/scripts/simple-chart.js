(function ($) {
    $.fn.simpleChart = function (options) {
        ...
        if (config.type === 'column') {
            var tcol = 100 / config.item.value.length;
            itemWidth = (100 - config.item.value.length * barMargin - barMargin) / config.item.value.length;
            itemHeight = setItemSize();
            for (i = 0; i < config.item.value.length; i++) {
                if (i==0)
                {
                    barColor = 'background:linear-gradient(0deg, rgba(34,193,195,1) 0%, rgba(113,115,191,1) 28%, rgba(209,143,144,1) 66%, rgba(254,130,129,1) 80%)';
                }
                else if(i==1)
                {
                    barColor = 'background:linear-gradient(0deg, rgba(236,230,10,1) 0%, rgba(160,186,102,1) 36%, rgba(83,172,80,1) 62%)';
                }
                else if(i==2)
                {
                    barColor = 'background:linear-gradient(0deg, rgba(218,216,202,1) 0%, rgba(131,187,152,1) 36%, rgba(0,123,123,1) 62%)';
                }
                else if(i==3)
                {
                    barColor = 'background:linear-gradient(0deg, rgba(218,216,202,1) 0%, rgba(0,147,147,1) 36%, rgba(0,204,153,1) 64%)';
                }
                else
                {
                    barColor = getBackgroundColor();
                ...
                template += '</div>';
                template += '<div >';
                template += '<span class="sc-label" style="left:' + ((itemWidth + barMargin) * i+4) + '%;width:' + (tcol) + '%;">' + config.item.label[i] + '</span>';
                template += '</div>';
